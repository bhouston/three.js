 ...
 * ends in a plain linear layer (see ../neural-texture/NeuralTextureModel.js
 * / NeuralTextureGPUModel.js). Consumers (currently
 * ../neural-material/NeuralMaterialFormat.js's per-channel `activation`
 * metadata) apply these as a post-processing step on the raw per-channel
 * output `z`, both at inference time
 * (../neural-material/NeuralMaterialNodeMaterial.js) and inside the GPU
 * training kernel (../neural-texture/NeuralTextureGPUComputeTSL.js), so the
