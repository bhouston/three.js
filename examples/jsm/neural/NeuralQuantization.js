...
import { encodeUint8Base64, decodeUint8Base64 } from './NeuralBinaryCodec.js';
...
function resolveQuantizationConfig( options = {} ) {
	...
	if ( QUANTIZATION_SCHEMES[ mode ] === undefined ) {
		...
		throw new Error( `THREE.NeuralQuantization: quantization.mode must be one of [${ VALID_MODES.join( ', ' ) }], got "${ mode }".` );

	}

	if ( VALID_TARGETS.includes( target ) === false ) {

		throw new Error( `THREE.NeuralQuantization: quantization.target must be one of [${ VALID_TARGETS.join( ', ' ) }], got "${ target }".` );

	}

	if ( target !== 'latents' && mode !== 'none' ) {

		throw new Error( `THREE.NeuralQuantization: quantization.target "${ target }" is not yet implemented - only "latents" is currently supported.` );
	...
	if ( range !== 'auto' ) {
		...
		if ( isRangeTuple === false ) {
			...
			throw new Error( 'THREE.NeuralQuantization: quantization.range must be "auto" or a [min, max] tuple with min <= max.' );
	...
	if ( typeof perLevel !== 'boolean' ) {
		...
		throw new Error( 'THREE.NeuralQuantization: quantization.perLevel must be a boolean.' );
 ...
 * - see NeuralTextureGPUModel.js/NeuralAppearanceGPUModel.js's
 ...
 * NeuralTextureTrainer.js for how often this actually runs).
 *
 ...
 * `setQuantizationRange` on NeuralTextureGPUModel.js/
 * NeuralAppearanceGPUModel.js) - shared by NeuralTextureTrainer.js/
