...
/**
 * Temporal Reprojection Anti-Aliasing Shader @bhouston
 */
...
vec3 find_closest_fragment_9tap(const in vec2 uv) {
	...
	const vec3 offset = vec3(1.0, -1.0, 0.0);  // TODO: Combine texelSize into offset to simplify code below.
...
vec4 accumulateTRAA( const in vec2 uv, const in vec2 screenSpaceVelocity ) {
...
void main() {
	...
	if( c_frag.z >= 1. ) {
		...
		gl_FragColor = accumulateTRAA( vUv, screenSpaceVelocity );
