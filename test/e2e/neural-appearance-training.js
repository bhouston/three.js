...
// `growthFactor`/`baseResolution` size the shared multiresolution latent
...
// use a coarser base resolution with a larger growth factor (reaching a finer
...
async function runTrainingCase( page, testCase ) {
	...
	const growthFactorParam = testCase.growthFactor ? `&growthFactor=${testCase.growthFactor}` : '';
	const baseResolutionParam = testCase.baseResolution ? `&baseResolution=${testCase.baseResolution}` : '';
	await page.goto( `http://localhost:${port}/examples/webgpu_materials_neural_appearance.html?test=${testCase.name}&autoTrain=1&noRotate=1&iterations=${testCase.iterations}&batchSize=256&hiddenSize=${testCase.hiddenSize || 32}${baseResolutionParam}${growthFactorParam}&seed=7`, {
...
function validateLosses( result, testCase ) {
	...
	// NeuralAppearanceFormat.js) regardless of baseResolution/growthFactor,
