...
export default QUnit.module( 'TSL', () => {
	...
	QUnit.module( 'logic and comparison operators', () => {
		...
		gpuTest( 'select() picks its if/else branch by condition, independent of the branch values', ( { assert } ) => {
			...
			// Works per-component on vectors too -- each lane picks independently.
			const cond = vec3( 1, 0, 1 ).greaterThan( vec3( 0, 0, 0 ) );
			assert.eq( select( cond, vec3( 1, 2, 3 ), vec3( 100, 200, 300 ) ), vec3( 1, 200, 3 ), 'select() is applied per-component on vectors' );
