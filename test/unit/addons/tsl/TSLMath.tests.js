...
export default QUnit.module( 'TSL', () => {
	...
	QUnit.module( 'math library', () => {
		...
		gpuTest( 'hyperbolic and inverse hyperbolic functions', ( { assert } ) => {
			...
			// Inverse hyperbolic functions are each checked as the true inverse
			// of their forward counterpart above, at the same independently
			// hand-computed values -- not merely round-tripped through the
			// TSL functions themselves.
		...
		gpuTest( 'all()/any() vector-to-scalar boolean reductions', ( { assert } ) => {
			...
			const allTrue = vec3( 1, 1, 1 ).greaterThan( vec3( 0, 0, 0 ) );
			const mixed = vec3( 1, -1, 1 ).greaterThan( vec3( 0, 0, 0 ) );
			const allFalse = vec3( -1, -1, -1 ).greaterThan( vec3( 0, 0, 0 ) );
