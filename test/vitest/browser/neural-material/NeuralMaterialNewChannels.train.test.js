...
import { NeuralMaterialNodeMaterial } from '../../../../examples/jsm/neural-material/NeuralMaterialNodeMaterial.js';
...
// render.test.js), but that a real `NeuralTextureTrainer` run against a
...
//
// This is the only test in the suite that drives `NeuralMaterialNodeMaterial
...
// generic (channel-agnostic) shape of NeuralMaterialSource.js/
// NeuralTextureTrainer.js.
describe( 'Addons > Neural > NeuralMaterial > new channels train end-to-end (real WebGPU)', () => {
	...
	it( 'fits every new channel jointly with finite, sharply-decreasing loss', async () => {
		...
		const { material: neuralMaterial, loss } = await NeuralMaterialNodeMaterial.fit( renderer, material, {
			resolution: 64,
			levels: 2,
			baseResolution: 8,
			growthFactor: 2,
		...
		// NeuralTextureTrainer.convergence.test.js) - not a strict "monotonic
