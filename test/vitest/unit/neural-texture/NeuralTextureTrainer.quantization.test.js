...
import { QUANTIZATION_SCHEMES, DEFAULT_QUANTIZATION_OPTIONS, computeLatentRanges } from '../../../../examples/jsm/neural/NeuralQuantization.js';

// Part 1: config plumbing (Task D) - mocks every GPU delegate point exactly
// like NeuralTextureTrainer.test.js, so `train()`'s orchestration (including
// the new QAT wiring: resolving `quantization`, forwarding it into
// `NeuralTextureGPUModel`, and freezing `trainer.quantizationRange` at the
...
describe( 'Addons > Neural > Neural-Texture > NeuralTextureTrainer QAT config plumbing (Phase 2)', () => {
	...
	it( 'forwards a resolved quantization option through to NeuralTextureGPUModel', async () => {

		const trainer = new NeuralTextureTrainer( { iterations: 2, quantization: { mode: 'uint8', range: [ - 2, 2 ] } } );
		const renderer = createRenderer();

		await trainer.train( { renderer, sourceTexture: {} } );

		const constructedWith = mocks.NeuralTextureGPUModel.mock.calls[ 0 ][ 0 ];
	...
	it( 'rejects an invalid quantization.mode before doing any GPU work', async () => {
		...
		const trainer = new NeuralTextureTrainer( { iterations: 2, quantization: { mode: 'int4' } } );
		const renderer = createRenderer();

		await expect( trainer.train( { renderer, sourceTexture: {} } ) ).rejects.toThrow( /quantization\.mode/ );
		expect( mocks.NeuralTextureGPUModel ).not.toHaveBeenCalled();

	} );

	it( 'mode: "none" never reads back the GPU latent buffer or freezes a range', async () => {

		const trainer = new NeuralTextureTrainer( { iterations: 5 } );
		const renderer = createRenderer();
	...
	it( 'mode: "uint8" freezes trainer.quantizationRange (and cpuModel.quantizationRange) from the GPU model at the end of training', async () => {
		...
		const trainer = new NeuralTextureTrainer( { iterations: 3, quantization: { mode: 'uint8' } } );
		const renderer = createRenderer();
...
describe( 'Addons > Neural > Neural-Texture > toy QAT training loop (Phase 2)', () => {
	...
	it( 'mode: "uint8" - loss decreases and final quantized latents land on <=256 discrete levels of the frozen range', () => {
		...
		// NeuralTextureTrainer.js's end-of-training freeze) and confirm every
