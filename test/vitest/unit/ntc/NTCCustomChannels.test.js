 ...
 * `NTCNodeMaterial`'s constructor runs) handles it correctly,
 * without editing anything in NTCFormat.js/NTCSource.js
...
describe( 'Addons > Neural > NeuralMaterial > custom channel composition', () => {
	...
	function buildCustomChannels() {
		...
		// scratch - NTCFormat.js has no idea "customTint" exists.
		const customTint = {
			key: 'customTint',
			size: 3,
			activation: 'sigmoid',
	...
	it( 'applyActive/applyConstant round-trip onto a plain target object, simulating NTCNodeMaterial\'s generic apply loop', () => {
		...
		// This mirrors exactly what NTCNodeMaterial's constructor
		...
		// NTCFormat.js's `constantEqualsDefault` pass) rather than
	...
	it( 'applyConstant is skipped entirely when the resolved value equals the channel\'s own default (a real regression: see NTCFormat.js\'s constantEqualsDefault doc comment)', () => {
